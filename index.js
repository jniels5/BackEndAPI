...
app.post('/stats/add/member', function(request,response) {
  ...
  connection.query('INSERT INTO Members set ?', entry, function (error, results, fields) {
        if(error) {
          ...
          connection.query('SELECT MemberID FROM MeD = tm.MemberID AND tm.TeamID = Teams.TeamID AND Teams.Semester = ' + mysql.escape(request.query.Semester) + ' ORDER BY m.MemberID', function (error, results, fields) {
        if(error) {
            response.json({all_select: "failed"});
        }
        else {
...
app.post('/map/insert/approvals/', function(request,response) {
    ...
    var entry = {
      ...
      Status: request.body.Status
 mbers WHERE FirstName = "' + entry.FirstName + '" AND LastName = "' + entry.LastName + '"', function (error, results, fields) {
          if(error) {
              response.json({
                membID_get: "Testing Failed",
                membID_err: error,
