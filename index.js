...
app.post('/edit/reserve', function(request,response) {
  var query = 'UPDATE Reservations SET Description = ' + mysql.escape(request.body.Description) + ', Email = ' + isNull(Email, mysql.escape(request.body.Email)) + ', TeamID = ' + isNull(TeamID, mysql.escape(request.body.TeamID)) + ' WHERE ReserveID = ' + mysql.escape(request.body.ReserveID);
  console.log(query);
  connection.query(query, function (error, results, fields) {
        if(error) {
            response.json({
